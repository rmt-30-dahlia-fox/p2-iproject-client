...
<template>
  ...
  <section :class="{flex: true, 'justify-center': !user || $route.path === '/profile', 'items-center': !user || $route.path === '/profile'}">
    ...
    <div v-if="user && !['/profile'].includes($route.path)" id="friend-list" class="w-1/5 p-4 shadow-lg h-100 bg-gray-50">
      <h1>People</h1>
      <div
	  @click.prevent="goGlobal()"
	  v-if="!['/global',].includes($route.path)"
	  class="flex p-2 pr-6 my-2 rounded-lg cursor-pointer hover:bg-gray-200"
	  >
	  <img
	      alt="Profile"
      ...
      <FriendCard
	  v-for="(data, i) in users.filter(u => u.id !== Number($route.params.id))"
	  :key="data.id"
	  :data="data"
	  />
    </div>
